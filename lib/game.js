...
class GameRoom {
    ...
    for (const t of this.teams) {
      if (!this.players.some(p => p.team === t.id)) {
        const p = {
          ...
          shopPoints: 0,
          accountKey: accountKey || (name || (t.emoji + ' ' + t.name + '战士')),
        };
        this.players.push(p);
  ...
  getState(forPid) {
    return {
      ...
      stats: forPid ? (globalStats[this.players.find(p => p.id === forPid)?.accountKey || this.players.find(p => p.id === forPid)?.name] || null) : null,
