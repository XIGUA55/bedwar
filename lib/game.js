...
class GameRoom {
  constructor(code, numTeams, io) {
    ...
    this.claims = {};         // playerId -> 'rock'|'paper'|'scissors' (public bluff)
    this.phaseEndTime = 0;    // ms timestamp when current phase expires
    this.actionEndTime = 0;   // ms timestamp when current action expires
    this._timer = null;       // current phase/action timer
  ...
  _startPrepareTimer() {
    this._clearTimer();
    this.phase = 'prepare';
    this.phaseEndTime = Date.now() + 5000;
    this._timer = setTimeout(() => this._startRPSTimer(), 5000);
  }

  _startRPSTimer() {
    this._clearTimer();
    this.phase = 'rps';
    this.phaseEndTime = Date.now() + 20000;
    this.broadcastAll();
    this._timer = setTimeout(() => {
      // Auto-submit random for players who didn't choose
      for (const p of this.alivePlayers()) {
        if (!this.rpsChoices[p.id]) this.rpsChoices[p.id] = ['rock','paper','scissors'][Math.floor(Math.random()*3)];
      }
      if (this.alivePlayers().length > 0) this.resolveRPS();
    }, 20000);
  }

  _startActionTimer() {
    this._clearTimer();
    this.actionEndTime = Date.now() + 15000;
    this._timer = setTimeout(() => {
      const actor = this.currentActor();
      if (actor) this.passAction(actor.id);
    }, 15000);
  }
  ...
  saveGameStats() {
    ...
    for (const p of this.players) {
      ...
      const rs = this.roundStats[p.id] || { kills: 0, bedsDestroyed: 0, finalKills: 0 };
  ...
  compileGameSummary() {
    ...
    // Reset the per-action timer
    if (this.phase === 'action') this._startActionTimer();
  }
      }
    }
  ...
  // ======== PUBLIC STATE ========
  getState(forPid) {
    const aliveNow = this.alivePlayers();
    return {
      code: this.code,
      phase: this.phase,
      teams: this.teams,
      players: this.players.map(p => {
        const teamAlive = aliveNow.filter(pl => pl.team === p.team).length;
        const isLastStand = p.alive && !p.respawning && !this.teamHasBed(p.team) && teamAlive === 1 && p.id === aliveNow.find(pl => pl.team === p.team)?.id;
        return { id: p.id, name: p.name, team: p.team, location: p.location, alive: p.alive, connected: p.connected !== false, respawning: p.respawning, pickaxe: p.pickaxe, enderPearl: p.enderPearl, tnt: p.tnt, shopPoints: p.shopPoints, isLastStand };
      }),
